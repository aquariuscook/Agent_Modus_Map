...
import type { ModelRoute, StatusCallback } from './provider-router-service.js';
...
export interface ProcessInterviewOptions {
  /** Callback for streaming lifecycle updates (e.g. SSE to frontend). */
...
export async function processInterviewMessage(
  interviewId: string,
  userMessage: string,
  options?: ProcessInterviewOptions,
...
- Always include the phase_data JSON block at the end of your response`;
  ...
  if (state.phase === 6 && !state.extracted.swarmConfig) {
    try {
      ...
      const config = await generateSwarmConfig(state, model!, route!, options?.onStatus);
  ...
  const result = await callGenerateText(
    { caller: 'interview', route: route!, model: model!, onStatus: options?.onStatus },
    {
      system: systemPrompt,
...
async function generateSwarmConfig(
  state: InterviewState,
  model: LanguageModel,
  route: ModelRoute,
  onStatus?: StatusCallback,
): Promise<GeneratedSwarmConfig> {
...
Output ONLY valid JSON matching this schema:
{
  ...
  const result = await callGenerateText(
    { caller: 'interview-config', route, model, onStatus },
    {
      prompt,
